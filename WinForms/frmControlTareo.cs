...
namespace WinForms
{
    public partial class frmControlTareo : Form
    {
        ...
        DataTable dtACTIVIDADES_TAREO = new DataTable();
        ...
        protected void listarCuadrilla()
        {
            ...
            if (dtResultado.Rows.Count > 0)
            {
                ...
                /////////////////////////////
                DetalleTrabajos();// LAS HH REGISTRADAS
                /////////////////////////////
        ...
        protected void DetalleTrabajos()
        {
            ...
            dtACTIVIDADES_TAREO.Clear();
            dtACTIVIDADES_TAREO = xobj.SEL_ASIGNACION_TAREAS_POR_FECHA_DISCIPLINA(cboEmpresa.SelectedValue.ToString(), cboCentroCosto.SelectedValue.ToString(), null, dateTareo.Value.Date.ToString("dd/MM/yyyy"), CAPATAZ, cboIngeniero.SelectedValue.ToString(), Convert.ToInt32(cboFile.SelectedValue));
            CANTIDAD_TAREAS = dtACTIVIDADES_TAREO.Rows.Count;

            if (dtACTIVIDADES_TAREO.Rows.Count > 0)
            {
                for (int i = 0; i < dtACTIVIDADES_TAREO.Rows.Count; i++)
                {

                    DataGridViewTextBoxColumn col = new DataGridViewTextBoxColumn();
                    col.Name = dtACTIVIDADES_TAREO.Rows[i]["ITEM_ACTIVIDAD"].ToString();
                    col.DataPropertyName = dtACTIVIDADES_TAREO.Rows[i]["IDE_TAREA"].ToString();
                    col.HeaderText = "Tarea " + dtACTIVIDADES_TAREO.Rows[i]["ITEM_ACTIVIDAD"].ToString();
                    col.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    col.Width = 67;
            ...
            if (lblEstado.Text == "1")
            {
                ...
                string dateString = dateTareo.Value.Date.ToString("dddd");
                dtResul = obj.SP_OBTENER_CUADRILLA_X_FECHA(cboCentroCosto.SelectedValue.ToString(), Convert.ToInt32(cboEmpresa.SelectedValue), cboCapataz.SelectedValue.ToString(), dateTareo.Value.Date.ToString("dd/MM/yyyy"),cboIngeniero.SelectedValue.ToString(), dateString);
            ...
            else
            {
                ...
                dtResul = objT.Lista_Personal_tareas_x_fecha(cboCentroCosto.SelectedValue.ToString(), Convert.ToInt32(cboEmpresa.SelectedValue), cboCapataz.SelectedValue.ToString(), dateTareo.Value.Date.ToString("dd/MM/yyyy"), dateString, cboIngeniero.SelectedValue.ToString());
                CANTIDAD_PERSONAL = dtResul.Rows.Count;
                if (dtResul.Rows.Count > 0)
                {
                    //datos vacios
        ...
        private void btnListarCuadrilla_Click(object sender, EventArgs e)
        {
            if (lblEstado.Text == "1")
            {
                ...
                if (respuesta == DialogResult.Yes)
                {
                    ...
                    dtResulTareo = obj.SP_CARGA_ULTIMA_CUADRILLA(cboCentroCosto.SelectedValue.ToString(), dateTareo.Value.Date.ToString("dd/MM/yyyy"),  cboCapataz.SelectedValue.ToString(),cboIngeniero.SelectedValue.ToString(),1,"");
                    if (dtResulTareo.Rows.Count > 0)
                    {
                        listarCuadrilla();
